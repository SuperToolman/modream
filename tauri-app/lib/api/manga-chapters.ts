...
export const mangaChaptersApi = {
  ...
  async getByMangaId(mangaId: number): Promise<MangaChapter[]> {
    return await http.get<MangaChapter[]>(`/manga/${mangaId}/chapters`);
  },
  ...
  getCoverUrl(
    ...
    const baseURL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080/api';
    return `${baseURL}/manga_chapter/${mangaId}/${chapterId}/cover?width=${width}&height=${height}&quality=${quality}`;
  },
};
